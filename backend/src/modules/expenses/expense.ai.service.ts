...
- description: a short, meaningful summary of what was bought — max 40 chars. Use item context to write a better summary than the merchant name alone (e.g. "idli vada tea" → "Morning Snacks", "beer chips" → "Evening Snacks", "milk eggs bread" → "Grocery Run"). If a well-known merchant is named AND no items are listed, use the merchant name (e.g. "Zomato", "Reliance Smart"). For non-item expenses use a concise label (e.g. "Auto Fare", "Electricity Bill"). CRITICAL: Never list the raw items here; never include cashback or reward details here.
...
- note: list EVERY item mentioned in the input — do NOT skip any item, even if it has no price or quantity. Format per item: "[qty] Item ₹Price" — include only the parts known for that item. If the user says "each" (e.g. "2 idli each 20rs"), append "each" after the price: "2 Idli ₹20 each". Examples: "2 idli each 20rs and 1 tea, 5 dosa" → "2 Idli ₹20 each, 1 Tea, 5 Dosa" | "burger 150 coke 50" → "Burger ₹150, Coke ₹50" | "2 eggs 3 bread" → "2 Eggs, 3 Bread" | "idli dosa vada" → "Idli, Dosa, Vada". CRITICAL: scan the entire input from start to finish and include every food/item word — never stop after the first item. Do NOT multiply or recalculate prices. If no specific items were mentioned at all, return null. Never mention cashback amounts here.
