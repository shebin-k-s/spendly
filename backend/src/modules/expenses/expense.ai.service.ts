...
export interface MonthAnalysisInput {
    ...
    heaviestDay: { date: string; net: number; topDescriptions: string[]; avgDayNet: number; spikeMultiple: number } | null;
...
${categoryBlock}`;
    ...
    private buildMonthAnalysisPrompt(input: MonthAnalysisInput): string {
        ...
        const heaviestDayLine = heaviestDay
            ? `Heaviest single day: ${heaviestDay.date} totaled ₹${heaviestDay.net} net (your average spending day this month was ₹${heaviestDay.avgDayNet}, so this was ${heaviestDay.spikeMultiple}x that), driven mainly by ${joinWithAnd(heaviestDay.topDescriptions)}.`
            : '';
...
3. HEAVIEST DAY OR TOP TRANSACTIONS: if the heaviest-single-day fact is given above, call it out by date and what drove it — and if it's meaningfully above the average day given alongside it (roughly 2x or more), explicitly frame it as a spike/outlier, not just "the biggest day" (e.g. "March 14 was a real spike — ₹2,400 in a single day, about 5x your typical ₹480 day, driven mostly by Rent and Groceries."). If it's only slightly above average, just state it plainly without calling it unusual. If no heaviest-day fact is available, mention the largest transaction(s) given above by description and amount instead, framed as a specific standout moment — if two or three are close in size, say so.
